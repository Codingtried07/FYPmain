...
def setFolders(args):
    ...
    if os.path.exists('models'):
        shutil.copytree('models', os.path.join(OUT_DIR, 'models'))
    if os.path.exists('train.py'):
        shutil.copy('train.py', OUT_DIR)
    if os.path.exists('./utils/model_utils.py'):
        shutil.copy('./utils/model_utils.py', OUT_DIR)
    if os.path.exists('cfgs'):
        shutil.copytree('cfgs', os.path.join(OUT_DIR, 'cfgs'))
...
def val(net, curr_epoch_num, val_loss_meters, dataloader_test, best_epoch_losses, LOG_FOUT, log_dir, best_cd_l1,
    ...
    if n_batches == 0:
        logging.info('No test data, skipping validation.')
